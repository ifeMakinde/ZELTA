...
async def get_intelligence(db: firestore.Client, uid: str) -> BrainResponse:
    ...
    recent = getattr(wallet_summary, "recent_transactions", []) or []
    for tx in recent[:20]:
        recent_transactions.append(_as_dict(tx))

    wallet_data = {
        "total_balance": wallet_summary.total_balance,
        "free_cash": wallet_summary.free_cash,
        "locked_total": getattr(wallet_summary, "locked_amount", 0.0),
        "locked_amount": getattr(wallet_summary, "locked_amount", 0.0),
    ...
    risk_tolerance = getattr(profile.financial, "risk_tolerance", None)

    profile_data = {
        "risk_tolerance": risk_tolerance,
        "risk_preference": risk_tolerance,  # backward compatibility
        "monthly_budget": getattr(profile.financial, "monthly_budget", None),
        "fee_obligations": getattr(profile.financial, "fee_obligations", None),
        "income_sources": getattr(profile.financial, "income_sources", None),
        "side_hustle_type": getattr(profile.financial, "side_hustle_type", None),
        "hostel_fee": getattr(profile.financial, "hostel_fee", None),
        "tuition_amount": getattr(profile.financial, "tuition_amount", None),
        "currency": getattr(profile.preferences, "currency", "NGN"),
        "language": getattr(profile.preferences, "language", "en"),
        "stress_alert_threshold": getattr(profile.preferences, "stress_alert_threshold", 60),
        "auto_lock_on_crisis": getattr(profile.preferences, "auto_lock_on_crisis", True),
        "show_bayse_prices": getattr(profile.preferences, "show_bayse_prices", True),
        "plain_english_mode": getattr(profile.preferences, "plain_english_mode", True),
        "notifications": _as_dict(getattr(profile, "notifications", {})),
    }

    brain_transaction_payload = {
        "transactions": recent_transactions,
        **(transaction_patterns or {}),
    }
    ...
    brain = await run_brain(
        ...
        transaction_patterns=brain_transaction_payload,
    )
...
def _map_to_brain_response(b: dict) -> BrainResponse:
    """
    Map the normalised brain dict to the typed BrainResponse Pydantic model.
    """
